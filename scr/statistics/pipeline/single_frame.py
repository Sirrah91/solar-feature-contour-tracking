...
def compute_frame_statistics(
    ...
    for region_key, region in regions.items():
        ...
        if region["is_ring"]:
            ...
            inner_mask = regions.get(inner, {}).get("total_mask", [])
            inner_empty = (
                    is_empty(inner_mask)
            ...
            if inner_empty:
                ...
                if can_copy:  # do not copy what is not needed (the contour-based results)
                    for q in image_map:
                        # Create a copy of the 'outer' results
                        source_data = results[q][outer].copy()

                        # We must remove any keys that originated from compute_contour_...
                        contour_keywords = ("fractal-dimension", "topology", "lengths", "flux-border")
                        keys_to_remove = [k for k in source_data.keys()
                                          if any(word in k.lower() for word in contour_keywords)]

                        for k in keys_to_remove:
                            source_data.pop(k)

                        results[q][region_key] = source_data
        ...
        region_lifetime = np.int32(compute_lifetime(sunspot, region["levels"][0]))  # lifetime of the outer boundary
