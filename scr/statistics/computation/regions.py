...
from scr.statistics.computation.masks import (
    contours_to_signed_fractional_mask,
...
def prepare_level_data(
    ...
    for level in sunspot:
        ...
        signed_masks = [
            contours_to_signed_fractional_mask(c, shape).astype(np.float64)
            for c in contours
            if not is_empty(c)
        ]
...
def build_regions(level_data: dict) -> tuple[dict, str]:
    ...
    if outermost_key:
        ...
        index = np.argmax(np.abs(signed_areas))
        sign = np.sign(signed_areas[index])

        envelope_mask = outer_data["signed_masks"][index] * sign

        envelope_contour = outer_data["densified_contours"][index]
        if sign < 0:
            envelope_contour = envelope_contour[::-1]

        regions["sunspot"] = {
            "masks": [envelope_mask],
            "total_mask": envelope_mask,
            "contours": [envelope_contour],
            "total_contour": envelope_contour,
