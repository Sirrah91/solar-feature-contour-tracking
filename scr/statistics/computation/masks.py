...
def compute_masks(
    ...
    fill_masks = [rasterize(c, shape, mode="surface", engine="cairo", use_orientation=True) for c in contours]
    border_masks = [rasterize(c, shape, mode="border", engine="skimage", use_orientation=True) for c in contours]
...
def build_refined_masks(
    ...
    if individual_masks is None:
        ...
        individual_masks = [
            rasterize(c, shape, mode="surface", engine="cairo", use_orientation=True)
            for c in contours
        ]
