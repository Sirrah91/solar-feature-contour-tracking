...
from scr.geometry.contours.shapes import union_contours
...
def process_frame_pairs(
    ...
    for frame, tids in frame_index.items():
        ...
        # --------------------------------
        # Build shapes for this frame only
        # --------------------------------
        geoms = []
        tids_list = []

        for tid in tids:
            contours = tracks[tid].get(frame)
            if not contours:
                continue
        ...
        # --------------------------------
        # Pair traversal (pruned)
        # --------------------------------
        for i, geom_a in enumerate(geoms):

            tid_a = tids_list[i]

            # STRtree gives bbox candidates
            ...
            for j in candidate_indices:
                ...
                # --------------------------------
                # Extra cheap bbox pruning (optional but good)
                # --------------------------------
        ...
        # --------------------------------
        # Explicit cleanup (important for large runs)
        # --------------------------------
        del tree, geoms, tids_list
...
def build_tracks_groups(
        tracks: Tracks,
        nesting: dict[TrackID, TrackID]
) -> dict:
    groups = nested_defaultdict(factory=list, depth=1)

    for tid in tracks:
        root = tid
        while root in nesting:
            root = nesting[root]
